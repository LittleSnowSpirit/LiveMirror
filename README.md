...
除 `/`、`/health` 和 `/auth/*` 外，默认业务接口都要求 Bearer 登录态。

### 已实现的功能面
- 账号注册、登录、刷新令牌、查询当前用户
- 功能模块注册表：返回后端挂载状态、前端导航入口和模块健康状态
- 上传音视频文件并创建分析任务
...
- 当前后端主入口使用 app factory + feature registry 挂载核心闭环路由，实验路由默认不作为主线暴露。
- 前端路由已改为懒加载，受保护页面会在未登录时跳转到登录页并保留返回地址。
- 默认转写 Provider 可用 mock 保证测试稳定；真实本地 Whisper/faster-whisper 仍需要按机器环境安装依赖和模型。
- 前端主导航只覆盖核心闭环，实验页面后续按模块巡检、补 API 封装、类型和冒烟测试后再转正。
